...
int try_move(int i, int x, int y, int nx, int ny)
{
	...
	if((pmap[ny][nx]&0xFF)==PT_MBMB && parts[i].type==PT_MBMB && parts[i].tmp == 1)
		e = 2;
...
//CUT HERE -- START
		(pmap[y][x]&0xFF)!=PT_ALUM &&
		(pmap[y][x]&0xFF)!=PT_BRAL &&
		(pmap[y][x]&0xFF)!=PT_CFRE &&
		(pmap[y][x]&0xFF)!=PT_LEAD &&
		(pmap[y][x]&0xFF)!=PT_UNWR &&
		(pmap[y][x]&0xFF)!=PT_STEL &&
//CUT HERE -- END
    ...
    for(i=start; i<(NPART-starti); i+=inc)
	     ...
	     else if(t==PT_SING)
		{
		    ...
		    parts[i].tmp = 1;
                ...
                if(parts[i].tmp==1 && pv[y/CELL][x/CELL]>1.0f)
                {
                    parts[i].vx += ptypes[t].advection*vx[y/CELL][x/CELL];
                    parts[i].vy += ptypes[t].advection*vy[y/CELL][x/CELL];
                }
	    ...
	    if(t==PT_WIFI && pv[y/CELL][x/CELL]>15.0f)
                t = parts[i].type = PT_BRMT;
	    if(t==PT_FRON && pv[y/CELL][x/CELL]>10.0f)
                t = parts[i].type = PT_FOIL;
	    if(t==PT_FOIL && pv[y/CELL][x/CELL]<-0.5f)
                t = parts[i].type = PT_FRON;
	    if(t==PT_ROCK && pv[y/CELL][x/CELL]>40.0f)
                t = parts[i].type = PT_STNE;
	    if(t==PT_PSTR && pv[y/CELL][x/CELL]>2.0f)
                t = parts[i].type = PT_DUST;
	    if(t==PT_GRND && pv[y/CELL][x/CELL]>20.0f)
                t = parts[i].type = PT_PGND;
	    if(t==PT_PGND && pv[y/CELL][x/CELL]<-10.0f)
                t = parts[i].type = PT_GRND;
	    if(t==PT_ALUM && pv[y/CELL][x/CELL]>40.0f)
                t = parts[i].type = PT_BRAL;
            ...
            if(t==PT_WATR || t==PT_ETRD || t==PT_ALUM || t==PT_BRAL || t==PT_CFRE || t==PT_LEAD || t==PT_UNWR || t==PT_STEL || t==PT_SLTW || t==PT_METL || t==PT_IRON || t==PT_RBDM || t==PT_LRBD || t==PT_BRMT || t==PT_PSCN || t==PT_NSCN || t==PT_NTCT || t==PT_PTCT || t==PT_BMTL || t==PT_SPRK|| t == PT_NBLE || t==PT_INWR)
            {
                nx = x % CELL;
                if(nx == 0)
                    nx = x/CELL - 1;
                ...
                {
                    if(t==PT_WATR || t==PT_ETRD || t==PT_ALUM|| t==PT_BRAL || t==PT_CFRE || t==PT_LEAD || t==PT_UNWR || t==PT_STEL || t==PT_SLTW || t==PT_METL || t==PT_IRON || t==PT_RBDM || t==PT_LRBD || t==PT_NSCN || t==PT_NTCT || t==PT_PTCT || t==PT_PSCN || t==PT_BRMT || t==PT_BMTL||t==PT_NBLE || t==PT_INWR)
                    {
                ...
                for(nx=-2; nx<3; nx++)
                    for(ny=-2; ny<3; ny++)
                                ...
                                x+nx<XRES && y+ny<YRES && (nx || ny))
                        {
                            ...
                            if((r>>8)>=NPART || !r)
                                continue;
                            if(((r&0xFF)==PT_METL || (r&0xFF)==PT_IRON || (r&0xFF)==PT_ETRD || (r&0xFF)==PT_ALUM || (r&0xFF)==PT_BRAL || (r&0xFF)==PT_CFRE || (r&0xFF)==PT_LEAD || (r&0xFF)==PT_UNWR || (r&0xFF)==PT_STEL || (r&0xFF)==PT_PSCN || (r&0xFF)==PT_NSCN || (r&0xFF)==PT_NTCT || (r&0xFF)==PT_PTCT || (r&0xFF)==PT_BMTL || (r&0xFF)==PT_RBDM || (r&0xFF)==PT_LRBD || (r&0xFF)==PT_BRMT || (r&0xFF)==PT_NBLE || (r&0xFF)==PT_INWR) && parts[r>>8].ctype!=PT_SPRK)
                            {
                                t = parts[i].type = PT_NONE;
            ...
            else if(t==PT_PLNT)
            {
			    ...
			    else if((r&0xFF)==PT_GRND ^ (r&0xFF)==PT_PGND && (1>rand()%20) && abs(nx+ny)<=2 && VINE_MODE)
			    {
				    int nnx = rand()%3 -1;
				    int nny = rand()%3 -1;
				    if(x+nx+nnx>=0 && y+ny+nny>0 &&
	    ...
	    else if(t==PT_VINE)
	    {
            ...
            else if(t==PT_NEUT)
            {
			    ...
			    if((r&0xFF)==PT_HYDR && 15>(rand()%1000))
                                parts[r>>8].type = PT_DEUT;
			    if((r&0xFF)==PT_ALCH && 15>(rand()%1000))
                                parts[r>>8].type = PT_DSTW;
			    if((r&0xFF)==PT_BASE && 15>(rand()%1000))
                                parts[r>>8].type = PT_ACID;
			    if((r&0xFF)==PT_FRON && 15>(rand()%1000))
                                parts[r>>8].type = PT_GAS;
			    if((r&0xFF)==PT_FOIL && 15>(rand()%1000))
                                parts[r>>8].type = PT_OIL;
			    if((r&0xFF)==PT_PSTR && 15>(rand()%1000))
                                parts[r>>8].type = PT_DUST;
			    if((r&0xFF)==PT_GRND && 15>(rand()%1000))
                                parts[r>>8].type = PT_PSTR;
			    if((r&0xFF)==PT_PGND && 15>(rand()%1000))
                                parts[r>>8].type = PT_SAND;
			    if((r&0xFF)==PT_MUD && 15>(rand()%1000))
                                parts[r>>8].type = PT_SLTW;
...
//CUT HERE -- START
	    if(t==PT_BASE)
	    {
                        ...
                        if(x+nx>=0 && y+ny>0 && x+nx<XRES && y+ny<YRES && (nx || ny))
			{
					...
					parts[r>>8].type = PT_NONE;
                            ...
                            if((r&0xFF)==PT_PLUT)
                        	{
                                	t = parts[i].type = PT_CNCT;
					parts[r>>8].type = PT_NONE;
                            	}
                            if((r&0xFF)==PT_ISOZ)
                        	{
                                	t = parts[i].type = PT_BIZR;
					parts[r>>8].type = PT_NONE;
                            	}
			}
	    }
	    ...
	    if(t==PT_CORE)
	    {
                        ...
                        if(x+nx>=0 && y+ny>0 && x+nx<XRES && y+ny<YRES && (nx || ny))
			{
                        	...
                        	{
					create_part(-1, nx, ny, PT_PLSM);
					create_part(-1, nx, ny, PT_PHOT);
					create_part(-1, nx, ny, PT_CORE);
                                	t = parts[i].type = PT_CORE;
					parts[r>>8].type = PT_THDR;
					pv[(y+nx)/CELL][(x+nx)/CELL] += 2560.0f;
                            	}
                         }
	    }
	    if(t==PT_HYDR)
	    ...
	    if(t==PT_MUD)
	    {
                        ...
                        if(x+nx>=0 && y+ny>0 && x+nx<XRES && y+ny<YRES && (nx || ny))
			{
			    ...
			    else if(((r&0xFF)==PT_WATR || (r&0xFF)==PT_SLTW || (r&0xFF)==PT_DSTW) && 1>(rand()%10000))
                            {
                                parts[r>>8].type = PT_NONE;
                            }
			}
				...
				for(nx=-2; nx<3; nx++)
					for(ny=-2; ny<3; ny++)
						if(x+nx>=0 && y+ny>0 && x+nx<XRES && y+ny<YRES && (nx || ny))
						{
							...
							if((r>>8)>=NPART || !r)
								continue;
							if(parts[r>>8].type!=PT_NONE && parts[r>>8].type!=PT_MBMB){
								parts[i].type = PT_NONE;
								goto killed;
							}
						}
			...
			for(nx=-2; nx<3; nx++)
				for(ny=-2; ny<3; ny++)
					if(x+nx>=0 && y+ny>0 && x+nx<XRES && y+ny<YRES)
					{
						...
						if((r>>8)>=NPART || !r)
							continue;
						if(parts[r>>8].type!=PT_NONE && parts[r>>8].type!=PT_DMND && parts[r>>8].type!=PT_CLNE && parts[r>>8].type!=PT_PCLN && parts[r>>8].type!=PT_BCLN && parts[r>>8].type!=PT_MBMB){
							int rad = 8;
							int nxi;
							int nxj;
							pmap[y][x] = 0;
							for(nxj=-(rad+1); nxj<=(rad+1); nxj++)
								for(nxi=-(rad+1); nxi<=(rad+1); nxi++)
									if((pow(nxi,2))/(pow((rad+1),2))+(pow(nxj,2))/(pow((rad+1),2))<=1){
										nb = create_part(-1, x+nxi, y+nxj, PT_MBMB);
										if(nb!=-1){
											parts[nb].tmp = 1;
											parts[nb].life = 50;
											parts[nb].temp = MAX_TEMP;
	    ...
	    if(t==PT_SEDS)
	    {
                        ...
                        if(x+nx>=0 && y+ny>0 && x+nx<XRES && y+ny<YRES && (nx || ny))
			{
                            ...
                            if((r>>8)>=NPART || !r)
                                continue;
                            if((r&0xFF)==PT_GRND || (r&0xFF)==PT_PGND || (r&0xFF)==PT_VINE)
                        	{
                                	t = parts[i].type = PT_VINE;
	    ...
	    if(t==PT_PROT)
	    {
                        ...
                        if(x+nx>=0 && y+ny>0 && x+nx<XRES && y+ny<YRES && (nx || ny))
			{
                        	...
                        	{
                                	t = parts[i].type = PT_NONE;
					parts[r>>8].type = PT_HYDR;
                            	}
			    parts[i].life = rand() % 5000 + 10;
			}
	    }
	    if(t==PT_ELEC)
	    {
	    ...
	    if(t==PT_TNT)
	    {
                        ...
                        if(x+nx>=0 && y+ny>0 && x+nx<XRES && y+ny<YRES && (nx || ny))
			{
			    ...
			    if((r&0xFF)==PT_WATR)
                        	{
					t = parts[i].type = PT_DYNA;
					parts[r>>8].type = PT_DYNA;
                            	}
			}
	    }
	    if(t==PT_BRAL)
	    {
                        ...
                        if(x+nx>=0 && y+ny>0 && x+nx<XRES && y+ny<YRES && (nx || ny))
			{
                            ...
                            if((r&0xFF)==PT_BRMT)
                        	{
                                	t = parts[i].type = PT_THRM;
					parts[r>>8].type = PT_THRM;
                            	}
			}
	    }
	    if(t==PT_LEAD)
	    ...
	    if(t==PT_COPY)
	    {
                        ...
                        if(x+nx>=0 && y+ny>0 && x+nx<XRES && y+ny<YRES && (nx || ny))
			{
                            ...
                            if((r>>8)>=NPART || !r)
                                continue;
                            r = pmap[y+ny][x+nx];
                            if((r&0xFF)!=PT_COPY&&(r&0xFF))
                        	{
					parts[i].type = parts[r>>8].type;
                            	}
			}
	    }
//CUT HERE -- END
                ...
                for(nx=-2; nx<3; nx++)
                    for(ny=-2; ny<3; ny++)
                                ...
                                x+nx<XRES && y+ny<YRES && (nx || ny))
                        {
                            ...
                            {
                                if((rt==PT_METL||rt==PT_IRON||rt==PT_ETRD||rt==PT_ALUM||rt==PT_BRAL||rt==PT_CFRE||rt==PT_LEAD||rt==PT_UNWR||rt==PT_STEL||rt==PT_BMTL||rt==PT_BRMT||rt==PT_LRBD||rt==PT_RBDM||rt==PT_PSCN||rt==PT_NSCN||rt==PT_NBLE)&&parts[r>>8].life==0 && abs(nx)+abs(ny) < 4)
                                {
                                    parts[r>>8].life = 4;
                                    parts[r>>8].ctype = rt;
                                    parts[r>>8].type = PT_SPRK;
...
//CUT HERE -- START
				if(t==PT_SPRK && (rt==PT_ALUM || rt==PT_BRAL || rt==PT_STEL) && parts[r>>8].life==0 &&
				...
				if(t==PT_SPRK && (rt==PT_CFRE || rt==PT_LEAD) && parts[r>>8].life==0 &&
					(parts[i].life<2 || ((r>>8)<i && parts[i].life<3)) && abs(nx)+abs(ny)<4)
				{
					if(parts_avg(r>>8, i,PT_INSL) != PT_INSL)
					{
							...
							parts[r>>8].ctype = rt;
                                ...
                                if(t==PT_SPRK && rt==PT_UNWR && parts[r>>8].life==0 &&
                                        (parts[i].life<3 || ((r>>8)<i && parts[i].life<4)) && abs(nx)+abs(ny)<4)
                                {
                                    if(parts_avg(r>>8, i,PT_INSL) != PT_INSL)
                                    {
...
killed:
		...
		if ((r&0xFF)==PT_BASE)  //If on base
                    parts[i].life -= 5;

		if ((r&0xFF)==PT_ALCH)  //If on Alcohol
                    parts[i].life -= 2;
